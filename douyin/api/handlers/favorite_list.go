...
func GetFavoriteList(c *gin.Context) {
	...
	videoList, err := rpc.GetFavoriteList(context.Background(), &core.DouyinFavoriteListRequest{
	...
	if err != nil {
		c.JSON(http.StatusOK, FeedResponse{
			Response: Response{StatusCode: 0, StatusMsg: "点赞视频列表加载失败"},
		})
	}
	...
	c.JSON(http.StatusOK, PublishedListResponse{
		Response:  Response{StatusCode: 0, StatusMsg: "加载点赞列表成功"},
		VideoList: Videos,
	})
}
